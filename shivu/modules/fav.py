...
async def fav(update: Update, context: CallbackContext) -> None:
    ...
    # 🔥 FIX: 0 padding hatakar clean ID nikal li
    ...
    try:
        ...
        character = None
        # 🔥 FIX: Ab 76, 076, 0076 sab perfectly match honge
        for c in user.get("characters", []):
            if isinstance(c, dict):
                cid_clean = str(c.get("id", "")).strip().lstrip('0') or '0'
                if cid_clean == req_id_clean:
                    character = c.copy()
                    break
        ...
        heading = to_small_caps("ARE YOU SURE YOU WANT TO MAKE THIS WAIFU YOUR FAVOURITE?")
...
async def handle_fav_callback(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if action == "fvc":
            ...
            if req_id_clean.isdigit():
                val = int(req_id_clean)
                q_ids.extend([val, f"{val:02d}", f"{val:03d}"])
