...
# ==========================================
tasks_collection = db['bot_tasks']           # Admin jo tasks add karega
user_tasks_collection = db['user_tasks']     # User ka task progress aur spend tracker
...
async def handle_referral(update: Update, context: CallbackContext):
    """
    Is command ko apne main /start handler ke andar call kar lena ya 
    is logic ko wahan copy paste kar dena.
    """
    ...
    if not user_task_data:
        # Naya user hai, 1000 welcome coins de do
        ...
        if context.args and context.args[0].startswith("ref_"):
            try:
                ...
                if referrer_id != user_id:
                    # Jisne invite kiya usko 25,000 coins do
                    await eco_collection.update_one(
                        {'id': referrer_id}, 
                        {'$inc': {'balance': 25000}}
                    )
                    try:
                        ref_msg = f"<b>{sc('🎉 Badhai ho! Tumhare invite link se kisi ne bot join kiya hai. Tumhe 25,000 coins mil gaye!')}</b>"
        ...
        # Naye user ka task DB initialize karo
        ...
        welcome_text = (
            f"<b>{sc('🎉 Welcome! Bot start karne ke liye tumhe 1,000 coins mile hain!')}</b>\n"
            f"<b>{sc('Aur coins kamane ke liye /tasks use karo.')}</b>"
        )
...
async def addtask(update: Update, context: CallbackContext):
    ...
    try:
        ...
        msg = (
            f"<b>✅ {sc('New Task Add Ho Gaya!')}</b>\n\n"
            f"<b>{sc('Name')}:</b> <b>{sc(name)}</b>\n"
            f"<b>{sc('Reward')}:</b> <b>{reward} {sc('Coins')}</b>\n"
            f"<b>{sc('Type')}:</b> <b>{sc(t_type.capitalize())}</b>\n"
            f"<b>{sc('Difficulty')}:</b> <b>{sc(difficulty.capitalize())}</b>"
        )
        ...
        error_msg = (
            f"<b>⚠️ {sc('Format galat hai!')}</b>\n"
            f"<b>{sc('Aise use karo')}:</b> <code>/addtask type | difficulty | reward | Name | URL(or None)</code>\n\n"
            f"<b>{sc('Example')}:</b>\n<code>/addtask daily | hard | 5000 | Join Our Channel | https://t.me/shivu</code>"
        )
...
async def tasks_cmd(update: Update, context: CallbackContext):
    ...
    if not all_tasks:
        await update.message.reply_text(f"<b>{sc('Abhi koi tasks available nahi hain! Baad mein check karna.')}</b>", parse_mode=ParseMode.HTML)
        return
    ...
    for task in all_tasks:
        ...
        # Inline buttons ke liye text (HTML tags nahi bas sc function)
        if is_completed:
            btn_text = f"✅ {sc(task['name'])} ({sc('Completed')})"
            cb_data = "task_ignore"
        elif task['difficulty'] == 'hard':
            btn_text = f"🔴 {sc(task['name'])} - {task['reward']} 🪙"
        ...
        if task.get('url') and not is_completed:
            ...
            keyboard.append([InlineKeyboardButton(f"{sc('Verify')} ✅", callback_data=cb_data)])
        else:
            keyboard.append([InlineKeyboardButton(btn_text, callback_data=cb_data)])
            
    bot_username = context.bot.username
    invite_link = f"https://t.me/{bot_username}?start=ref_{user_id}"
    keyboard.append([InlineKeyboardButton(f"👥 {sc('Invite Friends')} (25K {sc('Coins')})", url=f"https://t.me/share/url?url={invite_link}&text=Join%20this%20awesome%20bot!")])
    
    text = (
        f"<b>📋 <a href='tg://user?id={user_id}'>{sc('Task Dashboard')}</a></b>\n\n"
        f"<b><i>{sc('Task complete karo aur coins kamao! Daily tasks har raat reset hote hain.')}</i></b>\n\n"
        f"<b>{sc('Aaj Kharch Kiye Gaye Coins')}:</b> <b>{user_data.get('coins_spent_today', 0)} 🪙</b>"
    )
...
async def task_callback(update: Update, context: CallbackContext):
    ...
    if data == "task_ignore":
        await query.answer(sc("Ye task tum pehle hi complete kar chuke ho!"), show_alert=True)
        return
    ...
    if data.startswith("verify_task_"):
        ...
        if not task:
            await query.answer(sc("Ye task ab available nahi hai!"), show_alert=True)
            return
        ...
        if "spend" in task['name'].lower():
            ...
            if current_spent < required_spend:
                await query.answer(sc(f"Tumne abhi tak itne coins spend nahi kiye! (Spend limit: {current_spent}/{required_spend})"), show_alert=True)
                return
        ...
        await query.answer(sc(f"✅ Task Completed! Tumhe {task['reward']} coins mil gaye."), show_alert=True)
        ...
        log_data = {
            ...
            sc("ʀᴇᴡᴀʀᴅ"): f"<b><tg-emoji emoji-id=\"5472030678633684592\">💸</tg-emoji> {task['reward']:,} ᴄᴏɪɴs</b>",
